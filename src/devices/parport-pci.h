 ...
 *   nFault deasserted (PARPORT_STATUS_ERROR set) when the ring is empty,
 *   which the kernel parport stack recognizes as "no more data".
 */
 ...
 * SPP forward + IEEE 1284 nibble-mode reverse. Linux's parport_pc.c
 * picks it up via PCI ID match (vendor 0x9710 device 0x9900) and
 * exposes /dev/parport0 + /dev/lp0 + ppdev (PPNEGOT to nibble mode
 * enables bidirectional reads). EPP/ECP register windows return 0.
 *
