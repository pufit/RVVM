...
/*
 * Backend-agnostic 16550A register core. The bus wrapper (MMIO + FDT for
 * ns16550a, BAR-windowed multi-port for an Exar PCI combo card, etc.)
 * embeds a `uart16550_core_t`, drives byte-wide register access via
 * `uart16550_core_read/write`, and supplies an IRQ callback so the core
 * stays unaware of whether the line is wired INTx, PCI INTx, or MSI.
 */
